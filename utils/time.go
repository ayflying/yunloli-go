...
// 获取指定时间的0点时间戳
func (m *Mod) GetTimeDayZero(tm time.Time) time.Time {
...
func (m *Mod) GetWeekZero(now time.Time) time.Time {
...
// 计算两个时间间隔了几天
func (m *Mod) GetDayPass(startTime time.Time, t ...time.Time) int {
	// 获取时间的年、月、日
	year := startTime.Year()
...
// 获取下周一的时间
func (m *Mod) GetNextWeek(now time.Time) time.Time {
...
//	@param day 多少天后
//	@return time.Time
func (m *Mod) GetDayZeroTime(currentTime time.Time, day int) time.Time {
	// 加上指定天数的时间
...
//	@return time.Time
func (m *Mod) GetEndTime(startTime time.Time, _day ...int) time.Time {
	var day = 0
	if len(_day) > 0 {
		day = _day[0]
	}
...
func (m *Mod) GetDailyTimeList(time1 time.Time, time2 time.Time) (timeList []time.Time) {
